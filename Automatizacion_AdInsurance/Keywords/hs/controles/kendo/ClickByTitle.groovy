...
class ClickByTitle {
	@Keyword
	def clickElement(String testObjectId, String elementTitle) {
		KeywordUtil.logInfo("Selecting Title=" + elementTitle + ")")

		def elementTitle2 = "'"+elementTitle+"'";
		def strClick = '$("*[title=' + elementTitle2 + ']").click()';
		...
		if (!WebUI.verifyElementPresent(findTestObject(testObjectId), 20, FailureHandling.OPTIONAL)) {
			...
			KeywordUtil.markPassed('Selecting...' + elementTitle)
		}
	}
}
