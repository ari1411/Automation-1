...
WebUI.scrollToElement(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/table_riesgos'), 1)

totalSumAsegOrig = WebUI.getText(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/span_totalSumaAsegurada'))

print(totalSumAsegOrig)

totalSumAsegOrig = CustomKeywords.'hs.utils.convertString.stringFormatoNumerico'(totalSumAsegOrig)

print(totalSumAsegOrig)

/*
totalSumAsegOrigPars = (totalSumAsegOrig - '.')

while (totalSumAsegOrig != totalSumAsegOrigPars) {
    totalSumAsegOrig = totalSumAsegOrigPars

    totalSumAsegOrigPars = (totalSumAsegOrig - '.')
}

totalSumAsegOrig = totalSumAsegOrig.replace(',', '.')
*/

WebUI.click(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/tr_riesgos1'))

WebUI.click(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/td_sumaAsegurada1'))

cifraParcialOrig = WebUI.getAttribute(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/input_sumaAsegurada1'), 
    'value')

print(cifraParcialOrig)

nuevaCifra = ((cifraParcialOrig) as Float)

nuevaCifra = (nuevaCifra * 0.8)
...
nuevaCifra = df.format(nuevaCifra)
*/

print(nuevaCifra)

nuevaCifraString = nuevaCifra.toString()

print(nuevaCifraString)

WebUI.click(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/td_sumaAsegurada1'))

WebUI.clearText(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/input_sumaAsegurada1'))

WebUI.click(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/td_sumaAsegurada1'))

WebUI.setText(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/input_sumaAsegurada1'), nuevaCifraString)

WebUI.sendKeys(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/input_sumaAsegurada1'), Keys.chord(Keys.ENTER))

totalSumAsegPost = WebUI.getText(findTestObject('Emision_Hogar_Endoso_AumentoSumaAsegurada/span_totalSumaAsegurada'))

totalSumAsegPost = CustomKeywords.'hs.utils.convertString.stringFormatoNumerico'(totalSumAsegPost)

print(totalSumAsegPost)

/*
totalSumAsegPostPars = (totalSumAsegPost - '.')

while (totalSumAsegPost != totalSumAsegPostPars) {
    totalSumAsegPost = totalSumAsegPostPars

    totalSumAsegPostPars = (totalSumAsegPost - '.')
}

totalSumAsegPost = totalSumAsegPost.replace(',', '.')

*/

cantidadOriginal = ((totalSumAsegOrig) as Float)

cantidadPosterior = ((totalSumAsegPost) as Float)

WebUI.verifyLessThan(cantidadPosterior, cantidadOriginal)
